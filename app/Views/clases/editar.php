...
<select class="form-select" id="id_instructor" name="id_instructor" required style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;">
    ...
    <?php foreach($instructores as $instructor): ?>
        ...
        ?>
        <option value="<?= $instructor['id']; ?>" <?= $instructor['id'] == $clase['id_instructor'] ? 'selected' : ''; ?>>
    ...
    <?php endforeach; ?>
            ...
            <tr>
                <td style="padding: 10px; vertical-align: top;">
                    ...
                    <input type="datetime-local" id="fecha_inicio" name="fecha_inicio" required style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;" value="<?= $clase['fecha_inicio']; ?>" required> 
                </td>
                <td style="padding: 10px; vertical-align: top;">
                    <label for="fecha_fin" style="display: block; font-weight: bold; margin-bottom: 5px;">Fin: Fecha y Hora</label>
                    <input type="datetime-local" id="fecha_fin" name="fecha_fin" required style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;" value="<?= $clase['fecha_fin']; ?>" required> 
                </td>
            </tr>
            <tr>
                <td colspan="2" style="padding: 10px; vertical-align: top;">
                    <label for="descripcion" style="display: block; font-weight: bold; margin-bottom: 5px;">Descripción</label>
                    <textarea id="descripcion" name="descripcion" required style="width: 100%; padding: 8px; border: 1px solid #ccc; border-radius: 4px;"><?= $clase['descripcion']; ?></textarea>
                </td>
            </tr>
            <tr>
